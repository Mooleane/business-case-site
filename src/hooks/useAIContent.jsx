            ...
            try {
                ...
                body: JSON.stringify({
                    ...
                    },
                    {
                        ...
                        "personalizedAdvice" should be an array of objects, where each object has "advice" (datatype:string, (containing personalized and non-generic advice that is targeted towards their preferences (reach for 5 sentences)), and a "resources" (datatype:array of strings, with a "name" (datatype:string, resource (a resource name)), and a "url" (datatype:string, url (url to the resource).
                        "trendingRoles" should be an array of objects, where each object has a "career" (datatype:string, career (but make sure the role/career you find is trending, and make sure the role/career provided does not match the previous list of careers, and is relevant to the current date ${currentDate.toDateString()})), and a "skills" (datatype:string, skills (such as a description of general skills separated by commas, then hyperfocused skills)).
                        "goals" should be an array of objects, where each object has a "name" (datatype:string, goal (reach for 5)) and a "date" (datatype:string, deadline (such as "August 5th, 2023" but be aware the current date is ${currentDate.toDateString()})).
                        ...
                        "matchedCareers" should be an array of career objects, filtered from the provided career list, that best match the user's profile preferences (also include career objects that are relevant to their work preferences). 
                        The structure of each object in the array must be identical to the source.
                        `
                    },
                    ],
                    temperature: 0,
